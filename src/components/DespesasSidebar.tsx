import {
  ...
  Repeat, Bell, BarChart3, Package, Car,
} from "lucide-react";
...
import { useDespesasPermissions, DespesasAba } from "@/hooks/useDespesasPermissions";
import {
...
const moduleMenuItems: MenuItem[] = [
  ...
  { title: "Veículos", url: "/despesas/veiculos", icon: Car, aba: "veiculos" },
  ...
  { title: "Perfil", url: "/despesas/perfil", icon: UserCircle },
  { title: "Ajuda", url: "/despesas/ajuda", icon: HelpCircle },
];

const adminMenuItems: MenuItem[] = [
  { title: "Permissões por Aba", url: "/despesas/permissoes", icon: Shield },
  { title: "Usuários", url: "/despesas/usuarios", icon: Users },
  { title: "Auditoria", url: "/despesas/auditoria", icon: History },
];

const roleLabels: Partial<Record<AppRole, string>> = {
  super_admin: "Super Admin", admin: "Administrador", manager: "Gerente",
  supervisor: "Supervisor", collaborator: "Colaborador", broker: "Corretor",
};
const roleIcons: Partial<Record<AppRole, React.ReactNode>> = {
  super_admin: <Crown className="h-3 w-3" />, admin: <Crown className="h-3 w-3" />,
  manager: <Briefcase className="h-3 w-3" />, supervisor: <Briefcase className="h-3 w-3" />,
  collaborator: <User className="h-3 w-3" />, broker: <User className="h-3 w-3" />,
};
...
export function DespesasSidebar() {
  ...
  const { podeVer, loading: permsLoading } = useDespesasPermissions();

  const loading = roleLoading || systemLoading || permsLoading;

  const handleLogout = async () => {
    const { error } = await supabase.auth.signOut();
    if (error) toast.error("Não foi possível sair. Tente novamente.");
    else { toast.success("Logout realizado!"); navigate("/auth"); }
  };

  const visibleModuleItems = hasSystemAccess("despesas")
    ? moduleMenuItems.filter((item) => !item.aba || podeVer(item.aba))
    : [];
      ...
      <SidebarContent>
        <div className="px-4 py-3 border-b border-sidebar-border">
          <div className="flex items-start gap-3">
            <img src={executLogo} alt="Execut" className="h-10 w-10 object-contain flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <h2 className="text-base font-bold text-sidebar-foreground leading-tight">Gestão de Despesas</h2>
              {loading ? (
