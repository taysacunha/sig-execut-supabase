-- 1) Backfill vendas com team_id NULL usando a equipe atual do corretor
UPDATE sales s
SET team_id = sb.team_id
FROM sales_brokers sb
WHERE s.team_id IS NULL
  AND s.broker_id = sb.id
  AND sb.team_id IS NOT NULL;

-- 2) Recriar função get_sales_team_vgv_ranking_flexible para usar sales.team_id e incluir corretores inativos
...
-- 3) Recriar função get_sales_broker_vgv_ranking_flexible para incluir corretores inativos
