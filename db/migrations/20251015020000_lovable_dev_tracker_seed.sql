-- IDEMPOTENCY: only seed if dev_tracker is empty
-- Re-running this migration on a populated table is a no-op.
DELETE FROM public.dev_tracker WHERE FALSE;  -- noop, ensures table exists
DO $seed$
BEGIN
  IF EXISTS (SELECT 1 FROM public.dev_tracker LIMIT 1) THEN
    RAISE NOTICE 'dev_tracker already seeded, skipping';
  ELSE
